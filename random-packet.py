...
def enviar_paquete(destino, tipo_paquete, numero_paquetes, iface=None):
    """
    ...
    for i in range(numero_paquetes):
        puerto_aleatorio = random.randint(1, 65535)  # Generar puerto aleatorio

        if tipo_paquete.upper() == "ICMP":
            paquete = IP(dst=destino)/ICMP()/payload
        elif tipo_paquete.upper() == "TCP":
            paquete = IP(dst=destino)/TCP(dport=puerto_aleatorio)/payload
        elif tipo_paquete.upper() == "UDP":
            paquete = IP(dst=destino)/UDP(dport=puerto_aleatorio)/payload
        else:
    ...
    print(f"Se enviaron {enviados} paquetes a {destino} usando {tipo_paquete.upper()} en puertos aleatorios.")
...
if __name__ == "__main__":
    ...
    enviar_paquete(args.destino, args.tipo_paquete, args.numero_paquetes, args.iface)
